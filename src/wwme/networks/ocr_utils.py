...
class SpatialGather_Module(nn.Module):
    ...
    def forward(self, feats, probs):
        ...
        batch_size, c, _, _ = probs.size(0), probs.size(1), probs.size(2), \
            probs.size(3)
        feature_save= feats.cpu().detach().numpy()
        np.save(f'{cfg.RESULT_DIR}/features.npy', feature_save)
        ...
        #with open('feature_ocr.txt','a') as file:
            #print(np.array(ocr_context.cpu().detach().numpy).shape)
            
        context = ocr_context.cpu().detach().numpy()
        np.save(f'{cfg.RESULT_DIR}/ocr_context.npy', context)
